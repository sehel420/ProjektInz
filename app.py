...
## GREDDY HEURISTICS USED FOR CREATING RULES USING INFORMATION GAIN AS RULE MEASURE, GENERATION OF RULES PART
def calculate_entropy(df: pd.DataFrame, decision_col: str) -> float:
    if df.empty:
        return 0
    probabilities = df[decision_col].value_counts(normalize=True)
    
    entropy = -np.sum(probabilities * np.log2(probabilities + 1e-9))
    return entropy

def information_gain(df: pd.DataFrame, attr: str, value, decision_col: str) -> float:
    parent_entropy = calculate_entropy(df, decision_col)
    
    left = df[df[attr] == value]
    right = df[df[attr] != value]
    
    weight_left = len(left) / len(df)
    weight_right = len(right) / len(df)
    
    ig = parent_entropy - (weight_left * calculate_entropy(left, decision_col) + 
                             weight_right * calculate_entropy(right, decision_col))
    return ig

def greedy_heuristic_decision_rules(df: pd.DataFrame) -> list:
    ...
    for _, row in df.iterrows():
        Q = set()
        temp_df = df.copy()
        while len(temp_df[decision_col].unique()) > 1 and len(Q) < len(attributes):
            best_attr = None
            best_ig = -np.inf
            best_left = None
            for attr in attributes:
                if attr in Q:
                    continue
                value = row[attr]
                ig = information_gain(temp_df, attr, value, decision_col)
                if ig > best_ig:
                    best_ig = ig
                    best_attr = attr
                    best_left = temp_df[temp_df[attr] == value]
            if best_attr is None or best_ig <= 0:
                break
                
            Q.add(best_attr)
            temp_df = best_left
        if len(temp_df[decision_col].unique()) == 1:
            decision_value = temp_df[decision_col].iloc[0]
        else:
            decision_value = temp_df[decision_col].mode()[0]
            
        rule = {attr: row[attr] for attr in Q}
        rule['decision'] = decision_value
        rules.append(rule)
...
def show_rules(rules, file_name, title):
    ...
    Button(rules_window, text="Save to TXT", command=lambda: save_rules_to_txt(rules, file_name)).pack(pady=10)
...
def save_rules_to_txt(rules, file_name):
    ...
    for index, rule in enumerate(rules, start=1):
        temp_rule = rule.copy()
        decision = temp_rule.pop('decision', None)
        conditions = " and ".join([f"{k} = {v}" for k, v in temp_rule.items()])
        line = f"{index}. If {conditions} then decision = {decision}"
        lines.append(line)
...
def display_optimized_rules(optimized_rules, parent_window):
    ...
    Button(result_window, text="Save to TXT", command=lambda: save_optimized_rules_to_txt(optimized_rules)).pack(pady=10)
...
def save_optimized_rules_to_txt(optimized_rules):
    file_path = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text files", "*.txt")])
    if not file_path:
        return
    lines = []
    for index, (rule, _) in optimized_rules.items():
        if ", decision=" in rule:
            conditions_part, decision_part = rule.split(", decision=")
            conditions_list = conditions_part.split(", ")
            conditions_str = " and ".join([cond.strip() for cond in conditions_list])
            line = f"{index}. If {conditions_str} then decision = {decision_part.strip()}"
        else:
            line = f"{index}: {rule}"
        lines.append(line)
    with open(file_path, "w", encoding="utf-8") as f:
